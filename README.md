...
· 多Agent编排与ReAct决策（LangGraph）：基于 LangGraph 构建对话式“Supervisor + Experts”任务编排架构，将知识调研、常规办公、算法实现、实验执行和独立审查拆分为 5 类 Experts；Supervisor 负责任务拆解/委派/汇总，而 Experts 在独立上下文和角色工具白名单内运行局部 ReAct，通过“决策—执行—观察”循环动态推进任务；额外设计结构化实验上下文补全、执行权限预检、调用预算、重复调用检测等机制，减少工具调用冲突、上下文污染和无效循环。

· 分层记忆与工业知识增强（SQLite + RAG）：按信息生命周期划分单轮记忆、会话记忆、长期记忆，分别承载单轮ReAct轨迹、多轮任务状态、跨会话可复用经验，并将数据画像、实验规格和任务状态结构化保存并绑定当前会话，避免上下文歧义和历史结果误用；面向工业模型开发构建专业知识库，使用 SQLite 持久化由文献、实验等可追溯来源证据组成的知识体系，结合向量检索和字段检索召回相关知识，为算法选择、参数推荐、失败诊断和结果复盘提供可追溯知识支持。

· 工业Skills与实验闭环（Agent Skills + MCP）： 围绕工业模型开发构建8个 Agent Skills，覆盖资源搜索、组件添加、数据诊断、实验构建、方案探索与结果报告等，统一约束触发条件、结构化输入、工具序列、完成标准及异常恢复，通过 Function Calling 编排约 100 个本地以及外接 MCP Tool；训练进程由 Worker 异步执行并回写指标日志，未达到用户目标时，系统依据实验结果进行失败诊断、配置修复或生成新的候选实验，形成“数据诊断—组件准备—实验执行—结果审查—迭代探索”闭环。
...
· 抗扰参数高效微调（TCD-LoRA）： 提出Task-Corruption Decoupling LoRA算法，在Q/V投影层并联任务与扰动 LoRA，通过纯净-受扰双视图训练、预测一致性与子空间正交约束解耦任务与扰动语义；小样本7种混合受扰工况下均优于其余 8 种 LoRA 方法，rank=256 时以 28.62% 的微调参数量接近全微调准确率（90.52% vs 91.57%），较同秩 LoRA提升 13.42%。

· 重参数化离线部署（ONNX + INT8）： 推理前移除扰动 LoRA 并将任务 LoRA 无损合并至主干，完成 Torch/ONNX 数值对齐及Win 11 CPU与Atlas 200 NPU双平台部署与延迟、吞吐和内存测试。Win11 INT8 与 Ascend W8A8 分别压缩体积 72.18% 和 39.92%，Rank=64 量化前后准确率均保持 87.67%；Ascend 实板 p50 延迟46.65 ms，batch=8吞吐量23.40 img/s。
